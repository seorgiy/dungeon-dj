...
7. **Опционально.** Установите дополнительный компонент foobar для проигрывания музыки прямо из ютубных видео
8. **Опционально.** Настройте сам [foobar](#foobar-settings), чтобы было максимально удобно
...
Но даже более того, вы можете сохранить ютубовое видео как файл (грубо говоря, ссылку на видео), чтобы плеер мог в будущем воспроизводить его как обычный трек из папи.

Нажмите правой кнопой на видео в списке из ютуба → foobar2000 → File Operations → Copy to → выберите нужную папук с музыкой. Назовите трек как удобно, но обязательно добавьте расширение `.foo_youtube` в конец имени файла.
...
На самом деле этот проект я сделал для использования с кастомной клавой. С помощью [VIA](https://www.caniusevia.com/) или [Vial](https://get.vial.today/) вы можете стать **настоящим Dungeon DJ**. Лично я использую DOIO KB16-01:
